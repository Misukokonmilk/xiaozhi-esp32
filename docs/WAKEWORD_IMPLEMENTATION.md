...
实际测试 0.6 在安静环境下可用，但在正常使用中偶尔会无法唤醒，灵敏度偏低。经过进一步调优，**最终采用 0.45**:

- `0.2`: 误触发严重，空调声、键盘声都会触发
- `0.6`: 偶尔无法唤醒，需要很清晰的发音才能识别
- **`0.45`**: 实测最佳平衡点，误触发极少，正常语音即可稳定唤醒

阈值在 `pack_assets_direct.py` 中配置，修改后需重新打包 assets.bin 并刷写。
...
**默认阈值 0.2 在真实环境中太敏感，0.6 偏严格。**

xiaozhi-assets-generator 默认的 0.2 阈值可能是为演示场景设计的，真实使用环境中空调噪音、键盘敲击、脚步声都可能触发。调高到 0.6 后误触发减少，但偶尔无法唤醒。经过反复测试，**0.45 是最佳平衡点**: 误触发极少，正常语音即可稳定唤醒。
