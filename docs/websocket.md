...
## 1.1 令牌刷新与自动重连机制（同步版本）

为提升稳定性与避免不必要的令牌刷新，两端实现统一采用如下策略：

- 手动断开（用户或业务主动触发）
  - 不进行自动重连与令牌刷新。
  - 将设备状态切换为 `idle`，等待后续显式操作。

- 非手动断开（网络抖动、服务器主动关闭等）
  - 若当前设备处于 `connecting` 或 `listening` 阶段：
    - 有本地持久化令牌：直接使用现有令牌进行自动重连，不触发刷新。
    - 无令牌（为空）：先执行登录流程获取令牌（HTTP 登录），完成后写入 NVS 并重连。
  - 若自动重连失败，设备回到 `idle`，等待后续错误分支处理或用户操作。

- 握手/网络错误（连接建立失败）
  - 在 `OnNetworkError` 回调中触发一次令牌刷新（HTTP 刷新）。
  - 刷新成功后用新令牌重试建立连接；若仍失败，回到 `idle` 并提示错误。

实现要点：
- 令牌持久化位置：`Settings("websocket").SetString("token", ...)`；读取用 `GetString("token", "")`。
- 刷新与登录使用已封装任务：`http_refresh_token_start_task(...)` 与 `http_login_start_task(...)`。
- 关闭回调中通过内部标志位区分“手动关闭”与“非手动关闭”（例如 `closing_by_user_`）。
- 刷新仅在“连接建立失败（握手/网络错误）”场景发生，避免普通断连就刷新导致的无效请求与资源占用。

该策略确保：
- 令牌刷新只发生在真正需要的时机（连接失败），符合最小干预原则。
- 普通网络抖动引起的断开可以快速自动重连，提升用户体验。
- 主动断开时不会触发不必要的重连或刷新，逻辑更清晰可控。
