...
// jpg/image_to_jpeg.h removed — requires esp_camera.h (not available without camera component)
...
bool LvglDisplay::SnapshotToJpeg(std::string& jpeg_data, int quality) {
    // Disabled: requires image_to_jpeg_cb / PIXFORMAT_RGB565 from jpg/ code,
    // which depends on esp_camera.h (unavailable without camera component).
    // EchoEar uses Emote display mode and does not need snapshot-to-JPEG.
    ESP_LOGW(TAG, "SnapshotToJpeg not available (camera component not present)");
    return false;
}
